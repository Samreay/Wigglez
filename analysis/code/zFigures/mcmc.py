...
class CambMCMCManager(object):
    ...
    def getTamParameterBounds(self, numBins=25, desiredArea=0.6827, uid=None, kind='cubic'):
        if uid is None: uid = self.uid
        res = {}
        string = "\n\n%s\n\\begin{align}\n" % uid
        if self.finalSteps.get(uid) is None:
    ...
    def plotResults(self, size=(15,15), filename=None, uids=None, plotLine=True, parameters=None, labels=None, styles=None, colors=None):
        ...
        for i in range(n):
            for j in range(i + 1):
                ...
                for ii, uid in enumerate(uids):
                    if i == j:
                        self.plotBars(ax, pi[0], False, i==n-1, uid=uid, color=colors[ii], style=styles[ii], linewidth=linewidth)
                    else:
                        self.plotContour(ax, pi[0], pj[0], j==0, i==n-1, uid=uid, plotLine=(plotLine and ii==0), color=colors[ii], style=styles[ii], levels=levels, linewidth=linewidth)
                if i == n - 1:
                    ax.set_xlabel(pj[3])
                if j == 0:
                    ax.set_ylabel(pi[3])
    ...
    def plotContour(self, ax, param1, param2, showylabel, showxlabel, levels=[0, 0.6827, 0.9545], linewidth=1, style='-', color="#666666", plotLine=True, contourf=False, ticks=4, uid=None):
        if uid is None: uid = self.uid
        if style == '-':
            style = 'solid'
        elif style == '--':
