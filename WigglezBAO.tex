...
\title{Measuring the anisotropic clustering of galaxies in WiggleZ: Cosmological constraints} %$H(z)$, $D_A(z)$ and $\Omega_c h^2$}
...
We set $\Omega_c h^2$ as a free parameter, and fix other values to the WizCOLA simulation fiducial cosmology, such that $\sigma_8 = 0.812$, $n_s=0.961$, $h = 0.705$. {\red **Just for testing, or for the full analysis?**}  We fix $\Omega_b h^2$ to $0.0226$, as $\Omega_b h^2$ is well constrained by CMB data and variations even up to $5\sigma$ are negligible to the BAO model when testing Flat $\Lambda$CDM cosmology. This value is consistent with the WizCOLA simulation value $\Omega_b h^2 = 0.02266$. 
...
\end{align}
this requires knowledge of the power of the spectrum, which is marginalised over and thus unavailable.
...
\end{align}
We take into account galaxy bias $b$ by $\xi_{\rm galaxy}(s) = b^2 \xi(s)$ and we follow \citet{BlakeDavis2011} who incorporate scale dependent bias {\blue derived from simulations to the WiggleZ data}, $B(s)$, into to the model, via
\begin{align}
...
The Kaiser effect {\blue from coherent infall} can be modelled simply in Fourier space \citep{Kaiser1987}:
